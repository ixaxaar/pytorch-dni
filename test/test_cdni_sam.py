...
def test_rnn_1():
  ...
  rnn = CDNI(rnn, optim=optimizer, target_size=T.Size([batch_size, input_size]))
  optimizer.zero_grad()

  input_data, target_output = generate_data(batch_size, length, input_size, cuda)
  target_output = target_output.transpose(0, 1).contiguous()

  output, (chx, mhx, rv), v = rnn(input_data, None, target=list(target_output))
...
def test_rnn_no_memory_pass():
  ...
  outputs = []
  for x in range(2):
    output, (chx, mhx, rv), v = rnn(input_data, (chx, mhx, rv), pass_through_memory=False, target=list(target_output))
    output = output.transpose(0, 1)
