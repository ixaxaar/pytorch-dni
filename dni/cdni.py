...
class CDNI(Altprop):
  ...
  def __create_grad_nets(self, module, output):
    ...
    self.grad_nets[id(module)] = self.grad_network(
        input_size=output.size(-1) + self.label_size,
  ...
  def _backward_update_hook(self):
    def hook(module, grad_input, grad_output):
      ...
      for dim in range(len(output_shape)-2):
        labels = labels.unsqueeze(1)
      labels = self.labels.expand_as(T.zeros([*output_shape[:-1], -1]))

      # pass through the grad net
      hx = self.__get_grad_hidden(module)
      concated = T.cat([output, labels], -1)
